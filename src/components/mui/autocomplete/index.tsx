...
export const AutocompleteBase = (
  props: UseAutocompleteProps<(typeof options)[number], false, false, false>,
) => {
  const {
  ...
  return (
    <section className='autocomplete'>
      <div  {...getRootProps()} ref={rootRef}>
        <InputBase placeholder='Search' id={'id'} ref={setAnchorEl} {...getInputProps()} />
      </div>
      ...
      { anchorEl && (
        <PopperBase 
          open={popupOpen} 
          anchorEl={anchorEl}>
          <ul className='list' {...getListboxProps()}>
            {(groupedOptions as typeof options).map((option, index) => {
              const optionProps = getOptionProps({ option, index });
              return <li {...optionProps}>{option.year} {option.label}</li>;
            })}
